...
import Hasher from '../../../typhoon-ui/app/utils/mimc5.js';
import { commitmentAndNullifierHash } from '../../../typhoon-ui/app/utils/depositUtils.js';
import vk from '../../../typhoon-ui/app/utils/verification_key.json' assert { type: "json" }
import { parseGroth16ProofFromObject, parseGroth16VerifyingKeyFromObject } from '../../../typhoon-ui/app/utils/parsingUtils.js';
...
export async function generateProofCalldata(proofsString, recipient) {
    await garaga.init();
    let proofsStringList = JSONInputStringToList(proofsString)
    ...
    for (let i = 0; i < proofsStringList.length; i++) {
        ...
        let receipt = await provider.waitForTransaction(proofString.txHash)
        let depositEvent = typhoon.parseEvents(receipt)[0]["typhoon::Typhoon::Typhon::Deposit"]

        let [commitment, nullifierHash] = await commitmentAndNullifierHash(proofString.secret, proofString.nullifier)

        let index = depositEvent.commitments.indexOf(commitment)

        let result = getRootPairingsDirections(depositEvent.insertedIndexs[index], commitment, proofString.day, depositEvent.subtreeHelper[index])
        let proofInput = {
            "root": BigInt(result.r),
            "nullifierHash": nullifierHash,
            "day": BigInt(proofString.day),
            "recipient": BigInt(recipient),
            "pool": BigInt(proofString.pool),
            "secret": BigInt(proofString.secret),
            "nullifier": BigInt(proofString.nullifier),
        ...
        let parsedProof = parseGroth16ProofFromObject(proof, publicSignals)
        ...
        console.log("call data ",groth16Calldata)
        callDatas.push(groth16Calldata)
    }
    return callDatas
}
...
function JSONInputStringToList(input) {
